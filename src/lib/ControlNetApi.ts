...
export class ControlNetApi {
	...
	public async getModels(): Promise<string[]> {
		const response = await this.sd.api.get<{ model_list: string[] }>("/controlnet/model_list");
	...
	public async getModules(): Promise<string[]> {
		const response = await this.sd.api.get<{ module_list: string[] }>("/controlnet/module_list");
