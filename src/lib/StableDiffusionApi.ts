...
const createScriptsWithCnUnits = async (initScripts: {} | undefined, controlNetUnit: ControlNetUnit[]) => {
...
export class StableDiffusionApi {
	...
	public async img2img(options: Img2ImgOptions): Promise<StableDiffusionResult> {
		const init_images = await Promise.all(options.init_images.map(async (image) => await toBase64(image)));
	...
	public async extraBatch(options: ExtraBatchOptions): Promise<StableDiffusionResult> {
		...
		const images = await Promise.all(options.images.map(async (image) => await toBase64(image)));
	 ...
	 */
	public async interrogate(image: Sharp, model: string): Promise<StableDiffusionResult> {
		const image_data = await toBase64(image);
	 ...
	 */
	public async getProgress(skipCurrentImage: boolean = false): Promise<Progress> {
		const response = await this.api.get<Progress>(`/sdapi/v1/progress?skipCurrentImage=${skipCurrentImage}`);
		return response.data;
	}
	 ...
	 * @returns {Promise<void>}
	 */
	public async setModel(name: string, findClosest: boolean = true): Promise<void> {
