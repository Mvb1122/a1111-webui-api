 ...
 */
export async function toBase64(image: Sharp, raw: boolean = false): Promise<string> {
	const header = "data:image/png;base64,";

	const result = raw ? (await image.raw().toBuffer()).toString("base64") : header + (await image.png().toBuffer()).toString("base64");
